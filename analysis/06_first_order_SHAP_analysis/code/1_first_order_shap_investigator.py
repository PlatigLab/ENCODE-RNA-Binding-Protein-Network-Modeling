...
import adjustText, shap, tqdm, weasyprint, great_tables
...
class FirstOrderShapInvestigator:
    ...
    def create_model_average_performance_table(self): 
        ...
        subset = self.hash_metadata.copy(deep=True)
        subset['Pearson r'] = np.sqrt(subset['holdout_r2_score'])

        df_rows = []
        for cell_line in self.cell_lines:
            row = {"Cell Line": cell_line}
            for model in subset["name"].sort_values().unique():
                model_subset = subset[
                    (subset["cell_line"] == cell_line) & 
                    (subset["name"] == model)
                ]
                assert len(model_subset) == 5, f"Exspected 5 models for {cell_line} and {model}, but got {len(model_subset)}"

                # Calculate mean and std of R2 and Pearson r across the 5 models
                r2_mean = model_subset["holdout_r2_score"].mean()
                r2_std = model_subset["holdout_r2_score"].std()
                pearson_mean = model_subset["Pearson r"].mean()
                pearson_std = model_subset["Pearson r"].std()
                
                # Format as "mean ± std"
                r2_formatted = f"{r2_mean:.3f} ± {r2_std:.4f}"
                pearson_formatted = f"{pearson_mean:.3f} ± {pearson_std:.4f}"
                
                row[f"{model}_R2_Formatted"] = r2_formatted
                row[f"{model}_Pearson_Formatted"] = pearson_formatted

            df_rows.append(copy.deepcopy(row))

        performance_df = pd.DataFrame(df_rows)
        ...
        col_labels = {
            col: "R²" if col.endswith("R2_Formatted") else "Pearson r" for col in performance_df.columns if col not in ["Cell Line"]
        }
        ...
        table = (
            ...
            .tab_spanner(
                label = great_tables.md("<span style='color:green; font-style: italic;'>XGBoost</span>"),
                columns = [col for col in performance_df.columns if col.startswith("XGBRegressor")]
            )
            .opt_table_font(
                font="Arial",
            )
            .tab_style(
                style=style.borders(
                    sides="all", color="black", weight="0.4px", style="solid"
                ), 
                locations=loc.body()
            )
            .tab_style(
                style = style.borders(
                    sides="all", color="green", weight="2px", style="solid"
                ), 
                locations = loc.body(rows = [0,1], columns = [3, 4])
            )
        )
    ...
    def run_fishers_test_across_confusion_matrices_for_dpsi_sign_vs_local_SHAP_sign(self, data_mode=None):
        ...
        # Compute global min and max log10 odds ratio across all data
        vals = results_df["(A*D / B*C) Odds Ratio"]
        vmin, vmax = vals.min(), vals.max()

        with plt.style.context("../../paper.mplstyle"):
            fig, axes = plt.subplots(nrows, ncols, figsize=(4.5 * ncols, 4.5 * nrows), dpi=150, squeeze=False, sharex=True, sharey=True)

            # Store the mappable objects for each row
            row_mappables = []
            ...
            for row_idx, cell_line in enumerate(cell_lines):
                ...
                for col_idx, delta_shap_thr in enumerate(delta_shap_thresholds):
                    ...
                    row_hm = hm  # Save the last heatmap in each row
                ...
                row_mappables.append(row_hm)

            # Add a single colorbar for all subplots
            fig.subplots_adjust(right=0.92)  # Make space for colorbar
            cbar_ax = fig.add_axes([0.90,  # left
                                    0.12,  # bottom
                                    0.02,  # width
                                    0.5])  # height
            plt.colorbar(row_mappables[0].collections[0], cax=cbar_ax)
            cbar_ax.set_title("Fisher's\nExact Test\nOdds Ratio", fontsize=16, pad=10)
            ...
            fig.supxlabel("FDR", fontsize=30, y=0.02, x=0.48, fontweight="bold")
            fig.supylabel(f"| {DPSI_SYMBOL} |", fontsize=30, x=0, y=0.41, fontweight="bold")
            
            plt.tight_layout(rect=[0, 0, 0.88, 1])

            plt.savefig(
                self.FIGURES["dpsi_vs_local_SHAP_fishers_exact_test_across_all_thresholds"], 
                dpi=600, 
                bbox_inches='tight'
            )
            plt.show()
